...
class InviteAddForm extends AbstractForm {
	...
	public function readFormParameters() {
		...
		if (isset($_POST['code'])) $this->code = $_POST['code'];
		if (isset($_POST['emailField'])) $this->emailField = StringUtil::trim($_POST['emailField']);
		...
		if (INVITE_CODE_OPTION != 'maynot') if (isset($_POST['method'])) $this->method = $_POST['method'];
	}
	...
	public function validate() {
		...
		if ($this->method == 'email') {
			...
			if (count($emails) > INVITE_EMAIL_LIMIT) throw new UserInputException('emailField', 'tooMany');
			...
			if (mb_strlen($this->subject) > 255) throw new UserInputException('subject', 'tooLong');
